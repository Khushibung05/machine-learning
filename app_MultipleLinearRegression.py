...
# page configuration #
slt.set_page_config("Multiple Linear Regression App",layout="centered")
...
# Visualizations #
slt.markdown('<div class="card">',unsafe_allow_html=True)
slt.subheader("Total Bill vs Tip (with multiple linear regression")
fig,ax=plt.subplots()
...
<div class="card">
    ...
    <p><b>co-efficiecent (Total Bill): </b>{model.coef_[0]:.2f}<br>
       <b>co-efficiecent (Group Size): </b>{model.coef_[1]:.2f}<br>
       <b>intercept: </b>{model.intercept_:.2f}</p>
    <p>Tip Depends upon <b>Bill Amount</b> and <b>Group Size</b></p>
</div> 
""",unsafe_allow_html=True)
...
bill = slt.slider("Total Bill ($):",float(df.total_bill.min()),float(df.total_bill.max()),30.0)
size=slt.slider("Group Size:",int(df["size"].min()),int(df["size"].max()),2)
